...
| S2 | Spoofing | Forged or confused JWT (`none`, another HMAC algorithm, wrong issuer/audience, overlong token) | L | H | exact HS256, required `iss/aud/exp/iat/nbf`, max lifetime, DB token version, server-side session inventory with per-device revocation checked on every request, tests | Documented signing-key rotation procedure |
